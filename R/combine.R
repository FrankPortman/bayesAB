...
combine <- function(bT1, bT2){
  A1 <- bT1$posteriors$Probability$A 
  A2 <- bT2$posteriors$Mean$A
  B1 <- bT1$posteriors$Probability$B
  B2 <- bT2$posteriors$Mean$B
  ...
  result$prior <- NULL
  result$posteriors[['Probability']] <- list(A = A1*A2, B = B1*B2)
