combine <- function(bT1, bT2, f = `+`, params, newName = 'Parameter') {
  ...
  A1 <- bT1$posteriors$params[1]; A2 <- bT2$posteriors$params[1]
  B1 <- bT1$posteriors$params[2]; B2 <- bT2$posteriors$params[2]
  
  results <- list()
  
  results$inputs <- list(A_data = list(bT1$inputs$A_data, bT2$inputs$A_data),
                         ...
                         n_samples = c(bT1$n_samples, bT2$n_samples))
  
  results$posteriors[[newName]] <- list(A = f(A1, A2), B = f(B1, B2))
  
  results$distribution <- 'combined'
